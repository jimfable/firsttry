#!/usr/bin/env python3
"""Fail-closed, retrying runner for exhaustive_v3.

Unexpected technical failures never become negative company verdicts. Each company
is retried from a fresh AuditState; if all attempts fail, the workflow fails.
"""
...
import exhaustive_v3 as strict
...
async def audit_once(auditor: strict.StrictAuditor, item: dict) -> dict:
    async with auditor.company_sem:
        ...
        if not auditor.credible_positives(state) and (
            official_count < 5 or not getattr(state, "verified_hosts", set())
        ):
    ...
    async with aiohttp.ClientSession(
        ...
        with output.open("w", encoding="utf-8") as handle:
            for completed, future in enumerate(asyncio.as_completed(tasks), start=1):
                ...
                handle.flush()
                print(
                    f"fail-closed shard {args.shard}: {completed}/{len(subset)} "
                    f"nr={result['nr']} verdict={result['verdict']} "
                    f"confidence={result['confidence']} attempts={result['runner_attempts']}",
                    flush=True,
                )
