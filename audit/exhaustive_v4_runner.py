#!/usr/bin/env python3
"""Fail-closed retrying runner for the authoritative v4 auditor."""
...
import exhaustive_v4 as strict
...
async def guarded(auditor: strict.StrictAuditor, item: dict) -> dict:
    ...
    for attempt in range(1, 4):
        try:
            ...
            result["implementation"] = "exhaustive_v4_fail_closed"
            return result
    ...
    async with aiohttp.ClientSession(
        ...
        with output.open("w", encoding="utf-8") as handle:
            for completed, future in enumerate(asyncio.as_completed(tasks), start=1):
                ...
                handle.flush()
                print(
                    f"v4 shard {args.shard}: {completed}/{len(subset)} nr={result['nr']} "
