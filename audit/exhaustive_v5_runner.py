#!/usr/bin/env python3
"""Fail-closed retrying runner for the authoritative v5 patch layer."""
...
async def guarded(auditor: strict.StrictAuditor, item: dict) -> dict:
    ...
    for attempt in range(1, 4):
        try:
            ...
            result["implementation"] = "exhaustive_v5_fail_closed"
            return result
    ...
    async with aiohttp.ClientSession(
        ...
        with output.open("w", encoding="utf-8") as handle:
            for completed, future in enumerate(asyncio.as_completed(tasks), start=1):
                ...
                handle.flush()
                print(
                    f"v5 shard {args.shard}: {completed}/{len(subset)} nr={result['nr']} "
