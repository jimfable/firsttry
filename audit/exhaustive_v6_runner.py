#!/usr/bin/env python3
"""Exhaustion runner for v5: all negative candidates receive every fallback.

For a negative decision this runner always executes search, current official-site
crawl, red-team synonym search, Wayback, browser rendering and Common Crawl.
Unexpected failures are retried from a fresh state and can never be converted
into a company verdict.
"""
...
import exhaustive_v5 as strict
...
async def audit_once(auditor: strict.StrictAuditor, item: dict) -> dict:
    async with auditor.company_sem:
        ...
        if not auditor.credible_positives(state):
            # Always render a candidate site for a prospective negative; this is
            # no longer conditional on static page count.
...
async def guarded(auditor: strict.StrictAuditor, item: dict) -> dict:
    ...
    for attempt in range(1, 4):
        try:
            ...
            result["implementation"] = "exhaustive_v6_all_fallbacks_fail_closed"
            return result
    ...
    async with aiohttp.ClientSession(
        ...
        with output.open("w", encoding="utf-8") as handle:
            for completed, future in enumerate(asyncio.as_completed(tasks), start=1):
                ...
                handle.flush()
                print(
                    f"v6 shard {args.shard}: {completed}/{len(subset)} nr={result['nr']} "
