#!/usr/bin/env python3
"""All-fallbacks, fail-closed runner for authoritative v7."""
...
import exhaustive_v7 as strict
...
async def guarded(auditor: strict.StrictAuditor, item: dict) -> dict:
    ...
    for attempt in range(1, 4):
        try:
            ...
            result["implementation"] = "exhaustive_v7_identity_locked_all_fallbacks"
            return result
    ...
    async with aiohttp.ClientSession(
        ...
        with output.open("w", encoding="utf-8") as handle:
            for completed, future in enumerate(asyncio.as_completed(tasks), start=1):
                ...
                handle.flush()
                print(
                    f"v7 shard {args.shard}: {completed}/{len(subset)} nr={result['nr']} "
